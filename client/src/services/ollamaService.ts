...
export const syncOllamaModels = async (): Promise<SyncResult> => {
  const response = await api.post('/api/ollama/models/sync');
  return response.data;
};

// Get all models from database
export const getOllamaModelsFromDB = async (): Promise<OllamaModel[]> => {
  const response = await api.get('/api/ollama/models/db');
  return response.data.models;
};

// Get active models from database
export const getActiveOllamaModels = async (): Promise<OllamaModel[]> => {
  const response = await api.get('/api/ollama/models/active');
  return response.data.models;
};

// Toggle model active status
export const toggleOllamaModelStatus = async (id: string, isActive: boolean): Promise<OllamaModel> => {
  const response = await api.put(`/api/ollama/models/${id}/status`, { is_active: isActive });
  return response.data;
};
